...
class _GoalsScreenState extends State<GoalsScreen> {
  ...
  Future<void> _processGoalInput(String text) async {
    ...
    try {
      ...
      double targetAmount = double.parse(amountStr);

      // Handle "50k" -> 50000
      if (text.toLowerCase().contains(RegExp(r'\d+k'))) {
        targetAmount *= 1000;
      }
      ...
      String title = text
          ...
          .replaceAll(
            RegExp(r'i want|i need|for a|for an|for|k ', caseSensitive: false),
            '',
          )
          .trim();
      if (title.isEmpty) {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('✅ Added Goal: ${goal.title}')));
      }
      await _loadData();
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Failed to parse goal: $e')));
      }
    } finally {
              ...
              children: [
                ...
                IconButton(
                  ...
                  onPressed: _isProcessingAI
                      ? null
                      : () => _processGoalInput(_textController.text),
                ),
              ],
            ),
          ),
